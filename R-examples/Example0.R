...
set.seed(1) #setting the seed for reproducible random numbers
param.z <- list(sex.ratio)
param.w = list(mean.fitness, d.d)

pop <- evolve(list(start1), n.gens, "dynamic", "map", recom.map, param.z = list(param.z), param.w = list(param.w))
...
set.seed(1) #setting the seed for reproducible random numbers
pop<-evolve(list(start1), n.gens, "additive", "map", recom.map, param.z = list(param.z), param.w = list(param.w))

# A second population is included to simulate two populations with different phenotypic optima
b1_2 <- 0.9 # phenotypic optima

param.z2 <- list(sex.ratio, fitness.pos, bvs, add.loci, e.v)
param.w2 <- list(b0,b1_2,b2,b3, d.v, add.loci)

set.seed(1) #setting the seed for reproducible random numbers
pop<-evolve(list(start1, start2), n.gens, "additive", "map", recom.map, param.z = list(param.z, param.z2), param.w = list(param.w, param.w2))
